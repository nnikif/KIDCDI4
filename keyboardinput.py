...
class KeyboardInput():
    ...
    def draw_window(self):
        ...
        tk.Button(self.window, text="<<Назад",
                  command=self.move_left).grid(column=0, row=2)
        tk.Button(self.window, text="Сохранить и закрыть",
                  command=self.close).grid(column=1, row=2)
        tk.Button(self.window, text="Вперед>>",
                  command=self.move_right).grid(column=2, row=2)
    ...
    def _radio_button4(self, master, answer_var):
        v = tk.IntVar()
        for i in range(4):
            tk.Radiobutton(master, text=answer_var[i], value=i, variable=v).grid(
