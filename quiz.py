...
#         self.window=self.launch_window(master)
    ...
    def _fill_window(self, master):
        ...
        nameEntered = ttk.Entry(
            nameL, width=34, textvariable=self.dict['name'])
        ...
        ttk.Entry(birthL, width=3, textvariable=self.dict["d_birth"]).grid(
            row=0, column=0, sticky="W")
        self.dict["m_birth"] = tk.IntVar()
        ttk.Entry(birthL, width=3, textvariable=self.dict["m_birth"]).grid(
            row=0, column=1, sticky="W")
        self.dict["y_birth"] = tk.IntVar()
        ttk.Entry(birthL, width=5, textvariable=self.dict["y_birth"]).grid(
            row=0, column=2, sticky="W")
        fillL = ttk.LabelFrame(datesL, text="Дата заполнения:")
        fillL.grid(column=1, row=0, sticky="W")
        self.dict["d_fill"] = tk.IntVar()
        ttk.Entry(fillL, width=3, textvariable=self.dict["d_fill"]).grid(
            row=0, column=0, sticky="W")
        self.dict["m_fill"] = tk.IntVar()
        ttk.Entry(fillL, width=3, textvariable=self.dict["m_fill"]).grid(
            row=0, column=1, sticky="W")
        self.dict["y_fill"] = tk.IntVar()
        ttk.Entry(fillL, width=5, textvariable=self.dict["y_fill"]).grid(
            row=0, column=2, sticky="W")

        addressV = ttk.LabelFrame(col1L, text="Адрес: ")
        addressV.grid(column=0, row=2, sticky="W")
        self.dict["address"] = tk.StringVar()
        ttk.Entry(addressV, width=35, textvariable=self.dict["address"]).grid(
            row=0, column=0)

        phoneV = ttk.LabelFrame(col1L, text="Телефон: ")
        phoneV.grid(column=0, row=3, sticky="W")
        self.dict["phone"] = tk.StringVar()
        ttk.Entry(phoneV, width=35, textvariable=self.dict["phone"]).grid(
            row=0, column=0)
        ...
        ttk.Entry(numbercV, width=2, textvariable=self.dict["numberofkids"]).grid(
            row=0, column=0)
        ...
        ttk.Entry(birthWV, width=2, textvariable=self.dict["weekofbirth"]).grid(
            row=0, column=0)
        ...
        ttk.Entry(parentsW, width=3, textvariable=self.dict["m_age"]).grid(
            row=1, column=1)
        self.dict["f_age"] = tk.IntVar()
        ttk.Entry(parentsW, width=3, textvariable=self.dict["f_age"]).grid(
            row=2, column=1)
        self.dict["g_age"] = tk.IntVar()
        ttk.Entry(parentsW, width=3, textvariable=self.dict["g_age"]).grid(
            row=3, column=1)

        m_edu = tk.StringVar()
        comboM = ttk.Combobox(parentsW, textvariable=m_edu,
                              values=fv.P_EDUCATION, state="readonly")
        ...
        f_edu = tk.StringVar()
        comboF = ttk.Combobox(parentsW, textvariable=f_edu,
                              values=fv.P_EDUCATION, state="readonly")
        ...
        g_edu = tk.StringVar()
        comboG = ttk.Combobox(parentsW, textvariable=g_edu,
                              values=fv.P_EDUCATION, state="readonly")
...
#         tk.Button(nameL,text="Ручной ввод теста RCDI",command=self.manualinputCDI).grid(row=0,column=3)
        ...
        whoBestL = ttk.LabelFrame(
            col2L, text="Что ценит воспитатель в детях: ")
        ...
        languagesL = ttk.LabelFrame(
            col3L, text="На каком языке говорят дома: ")
    ...
    def _radioB(self, master, answers):
        ...
        for i in range(len(answers)):
            tk.Radiobutton(master, text=answers[i], value=i, variable=v).grid(
                column=i % 2, row=2 + i // 2, sticky='W')
        return v
    ...
    def read_with_dates(self):
        ...
        try:
            dict_wd["birthday"] = datetime.date(
                dict_raw["y_birth"], dict_raw["m_birth"], dict_raw["d_birth"])

        except:
            messagebox.showinfo(
                "Ошибка даты", "Введите корректный день рождения")
            return
        try:
            dict_wd["day_filled"] = datetime.date(
                dict_raw["y_fill"], dict_raw["m_fill"], dict_raw["d_fill"])
        except:
            messagebox.showinfo(
                "Ошибка даты", "Введите корректный день заполнения анкеты")
            return
...
#             print(key, dict1[key])
        ...
        dict1['d_fill'], dict1["m_fill"], dict1["y_fill"] = read_datetime(
            dict1.pop("day_filled"))
        dict1["d_birth"], dict1['m_birth'], dict1["y_birth"] = read_datetime(
            dict1.pop("birthday"))
#         print(dict1)
