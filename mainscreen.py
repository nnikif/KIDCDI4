...
class WidgetRoot():
    ...
    def _setKidsList(self, master):
        ...
        tk.Button(master, text="Новый ребенок, тест KID",
                  command=self.runKID).grid(column=1, row=0)
        tk.Button(master, text="Новый ребенок, тест RCDI",
                  command=self.runRCDI).grid(column=1, row=1, sticky="N")
    ...
    def _setTestsList(self, master):
        ...
        newTSTB = tk.Button(master, text="Новый тест RCDI",
                            command=self.newRCDI)
    ...
    def showTests(self, id):
        ...
        for test in data:
            self.treeR.insert(
                "", 1000, text=test[0], values=test[1].strftime("%d/%m/%Y"))
...
#
    def showinTab(self, nameS):
        ...
        for person in data:
            self.tree.insert("", 100000, text=person[0], values=(
                person[1], person[2].strftime("%d/%m/%Y")))
