...
class QTest(Quiz, Test):
    def __init__(self, ttype):
        ...
        tk.Button(tab01, text="Загрузить из .pdf файла",
                  command=self.read_from_pdf).grid(column=0, row=7)
        tk.Button(tab01, text="Ручной ввод теста",
                  command=self.call_keyboard).grid(column=1, row=7)
        tk.Button(tab01, text="Сохранить данные",
                  command=self.save_data).grid(column=2, row=7)
